...
def main():
    ...
    ok, mtx, dist, _, _ = cv2.calibrateCamera(objpoints, imgpoints, (WIDTH, HEIGHT), None, None)
    if not ok:
    ...
    out = {
        ...
        "distortion_coefficients": {"rows": int(dist.size), "cols": 1, "data": dist.reshape(-1).tolist()},
