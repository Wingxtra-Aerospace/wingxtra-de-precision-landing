...
2. If `--databus-sniff` is set, probe candidate ports (`--databus-sniff-ports`, `mavlink_out.databus_candidate_ports`, `DATABUS_CANDIDATE_PORTS`) using local UDP bind-table inspection on localhost candidates.
...
2. Probe candidate ports conservatively (localhost bind-table inspection) to avoid UDP send false-positives.
