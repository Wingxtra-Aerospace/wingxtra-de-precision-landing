...
## Review changes in rc.2

Heartbeat age is measured from Linux kernel packet arrival (`SO_TIMESTAMPNS_NEW`), not from the later application read. The supported runtime requires Linux 5.1 or newer; an unavailable timestamp facility prevents opening the output endpoint. A wall-clock discontinuity that makes packet age invalid rejects that packet. Persistent freshness uses a monotonic clock. The router must send complete MAVLink messages in each UDP datagram; old partial datagrams are never joined to new bytes.

Receiving 64 datagrams without reaching an empty socket marks telemetry backlogged and suppresses output until a subsequent poll drains it. Setup operations poll telemetry before editing. Calibration saving and camera-dependent configuration changes recheck after their potentially slow work. First-time offline bench setup is permitted, but after any autopilot telemetry is observed a fresh disarmed heartbeat is required for further setup, even following endpoint reconfiguration. A stale disarmed state no longer unlocks setup. This remains a process-local operational guard; the service cannot prevent the autopilot from arming independently.

DataBus and router initialization succeed together or are closed together. Transport selection is explicit; no failure switches transports. A failed camera shutdown latches a restart requirement, clears previews/measurements and prevents another camera from being opened over an uncertain driver state. Shutdown still releases telemetry sockets. Docker health status reports health; `unless-stopped` restarts exited containers, and does not itself restart a running but unhealthy container. Diagnose/restart a hung driver during bench recovery.

Board imports require numeric 4x3 matrices and consistent corner winding. SVG export chooses the front face from that winding so y-up and y-down layouts both produce readable AprilTags without changing physical dimensions. Planar PnP can remain ambiguous in low-information views; a low fit error alone does not prove distance/origin accuracy. Single-tag operation, especially with an origin far outside the visible tag, requires independent physical checks across the intended envelope.
