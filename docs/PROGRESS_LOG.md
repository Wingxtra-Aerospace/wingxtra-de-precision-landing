...
## 2026-09-14 — F01–F04/G01–G05/L04: review the resolved merges

- The user requested a merge-integrity and readiness review. Main `d6ed94a5474ba869b15299b1d4cfa310c816cc84` retains PR #36's tested source `176c9e0c3b761a158b88058049c0176d6bb68fe1`; only the five tracking documents differ. Its tree `23e7fe8f4f42aab858d56133adc263b39d32faaa` exactly matches final PR #37 head `0be3fd0ed15adbe9aa94f95b011c330f5b78aab4`. No conflict markers, whitespace errors or lost source/test/applet changes were found.
- [Workflow 34846268164](https://github.com/Wingxtra-Aerospace/wingxtra-de-precision-landing/actions/runs/34846268164) passes on that exact main commit: Python job `103982856299` executes 126 Python tests and 12 Lua API-stub scenarios, plus lint/format and browser checks. AMD64 `103983220517` and ARM64 `103983220530` reuse cached test-build layers and freshly pass startup/health/metadata/disabled-output checks. Earlier native runs under workflow 34841164832 each executed 126 tests; this distinction is retained in PROJECT_STATUS.
- Manifest job `103983887907` publishes the commit-qualified candidate with combined digest `sha256:95d5316e4f9752a960cb575dbd49dbe536d70ce2c0cb0caf784f6d3af01b22e7`. Exact run/artifact links are in the [merge review](PROJECT_STATUS.md#merge-integrity-review--2026-09-14). No physical installation is inferred.
- Rechecked the current checkout locally: 126 Python tests pass with two dependency deprecation warnings; Ruff lint/format, JavaScript syntax and 12 Lua API-stub scenarios pass. Reviewed current camera profiles, frame/clock handling, applet gates and packaging. No new merge-related runtime defect was identified; firmware-mode/recovery, measured timing/offset and hardware acceptance remain outstanding.
- Corrected tracker wording that implied qualification had started; tied the older archive-digest table explicitly to PR-head run 34830596708 and changed historical implementation wording to past tense. Pinned the initial source-review links to their actual baseline, aligned O01/O09 dependencies with F01's completed vendoring scope, and made the next actions follow D03's fixed-camera-first sequence. No architectural decision, task acceptance, flight setting or runtime behaviour changed.
- This documentation-only correction updates current status and evidence; F02–F03/G01–G05 remain IN PROGRESS. Next: R01–R04 inputs, version-matched fixed-camera SITL, measured CM4/Pixhawk commissioning, then the gated fixed/gimbal qualification sequence. Landmark comparison remains blocked on supplied material.

## 2026-09-14 — REF01/B01/T02/T16: inspect Landmark boot data and reopen multi-tag acceptance

- Reviewed the user-supplied `Precision landing.zip` against main `ab2f242790d324ef030602f0aa2a5bcbf940f8c5`. Archive SHA-256: `9f51f6bb9bf9588a5a16ff8735bd1409e5da9be1574792db05ed5bfdfabd7c37`. [LMBOOT-001](LANDMARK_REFERENCE_REVIEW.md) records input hashes, inventory, methods, environment and results. The ZIP has 404 files; CRC validation passes. Both initramfs inventories were inspected without executing supplied content. No Landmark application, active runtime setup or flight recordings were located.
- The four-tag A4 board passes the current loader. The fifteen-tag Fibonacci definition exceeds the existing 5 m marker-edge guard under Wingxtra's unit convention; physical units/active board remain unconfirmed. The 3280×2464 calibration fails Wingxtra's schema/metadata requirements and current height limit. No private calibration, geometry or licence values were committed, and no converter or limit change was implemented.
- B01 **DONE -> IN PROGRESS**: a bounded local diagnostic on Python 3.12.14 / NumPy 2.5.3 / OpenCV 4.13.0 rejects nine of eighteen noiseless depth/seed combinations for a fully synthetic five-tag board. All geometry, camera and pose inputs are published in LMBOOT-001, making the diagnosis independently retrievable without the restricted attachment. All corners are in-frame, positive-depth and large enough. The EPNP-RANSAC prefilter drops valid observations before IPPE; direct IPPE fits all observations with negligible residual. The attachment-defined A4 result remains corroborating evidence. This is new synthetic software evidence, not Landmark execution or a hardware/flight result. Earlier CI remains valid only for its exercised cases.
- Added the B01 correction/regression prerequisite to F05/M2 and clarified T02. REF01 stays BLOCKED with partial data review complete; O10 now names the missing root filesystem/application, active configuration and reference observations. No architectural decision, aircraft policy or numeric operating limit was selected.
- In response to the user's portability question, inspected only the licence structure for the report. Its serial format is consistent with a Pi hardware identifier and suggests a device-bound licence, but the verifier and transfer policy are absent. Enforcement, same-Pi card replacement and another-Pi operation remain untested; no licence values or modifications are included.
- Next: correct B01 while preserving outlier protection and verify the declared OpenCV environments; obtain the remaining reference material for T16. This change is documentation-only. No runtime, applet, tests, workflows, calibration, aircraft parameters or deployment was modified; no SITL, CM4 bench or flight qualification is claimed.
