...
- B01 **DONE -> IN PROGRESS**: a bounded local diagnostic on Python 3.12.14 / NumPy 2.5.3 / OpenCV 4.13.0 rejects six of twelve noiseless depth/seed combinations for the accepted A4 board. All corners are in-frame, positive-depth and large enough. The EPNP-RANSAC prefilter drops valid observations before IPPE; direct IPPE fits all observations with negligible residual. This is new synthetic software evidence, not Landmark execution or a hardware/flight result. Earlier CI remains valid only for its exercised cases.
